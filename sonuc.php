...
$result = mysqli_query($open, "SELECT * FROM `seyehat` WHERE  LOWER(`nerden`) LIKE LOWER('%$aranan%') OR LOWER(`nereye`) LIKE LOWER('%$aranan%') OR LOWER(`tarih`) LIKE LOWER('%$aranan%') OR LOWER(`kisi`) LIKE LOWER('%$aranan%') OR LOWER(`fiyat`) LIKE LOWER('%$aranan%') OR LOWER(`aciklama`) LIKE LOWER('%$aranan%') ");
if ($result == FALSE) {
...
<body>
    ...
    <section class="packages" id="packages">
        ...
        <h1 class="heading"> Arama Sonucu Çıkan <span>Yolculuklar</span> </h1>
        <div class="box-container">
        <?php
        foreach ($result as $sonuc) {
            $result = mysqli_query($open, "SELECT `name`, `surname`, `e-mail`, `tel` FROM `users` WHERE `id` = '".$sonuc["userID"]."' ");
            if ($result == FALSE) {
                exit(json_encode(["result" => NULL, "error" => ["code" => NULL, "message" => "An error occurred while selecting data on database"]]));
            }
        ...
        ?>
            <div class="box" data-aos="fade-up">
                ...
                <div class="content">
                    <h3 class="dest"><i class="fas fa-map-marker-alt"></i><?=$sonuc["nerden"]?>-<?=$sonuc["nereye"]?></h3>
                    <h3 class="date"><i class="fas fa-calendar"></i><?=$sonuc["tarih"]?></h3>
                    <p><?=$sonuc["aciklama"]?></p>
                    <p><b><?=$sonuc["kisi"]?> Kişi</b></p>
                    <div class="fiyat:"><b><?=$sonuc["fiyat"]?> TL</b></div>
                    <a href="fav-ekle.php?id=<?=$sonuc["id"]?>&userID=<?=$sonuc["userID"]?>" class="btn"><i class="fa fa-heart" aria-hidden="true"></i> fav ekle</a>
                    ...
                    <a href="rezervasyon-ekle.php?seyehatID=<?=$sonuc["id"]?>" class="btn"><i class="fa fa-calendar" aria-hidden="true"></i> rezervasyon</a>
