...
$result = mysqli_query($open, "SELECT `seyehatID` FROM `rezervasyon` WHERE `userID` = '".$userID."'");
if ($result == FALSE) {
    ...
    <section class="packages" id="packages">
        ...
        foreach ($result as $seyehatID) {
            ...
            if ($result == FALSE) {
                ...
                foreach ($result as $rezervasyon) {
                    $result = mysqli_query($open, "SELECT `name`, `surname`, `e-mail`, `tel` FROM `users` WHERE `id` = '".$rezervasyon["userID"]."' ");
                    if ($result == FALSE) {
                        exit(json_encode(["result" => NULL, "error" => ["code" => NULL, "message" => "An error occurred while selecting data on database"]]));
                    }
        ...
        <div class="box-container">
            <div class="box" data-aos="fade-up">
                ...
                <div class="content">
                    <h3 class="dest"><i class="fas fa-map-marker-alt"></i><?=$rezervasyon["nerden"]?>-<?=$rezervasyon["nereye"]?></h3>
                    <h3 class="date"><i class="fas fa-calendar"></i><?=$rezervasyon["tarih"]?></h3>
                    <p><?=$rezervasyon["aciklama"]?></p>
                    <p><b><?=$rezervasyon["kisi"]?> Kişi</b></p>
                    <div class="fiyat:"><b><?=$rezervasyon["fiyat"]?> TL</b></div>
                    ...
                    <a href="rezervasyon-ekle.php?seyehatID=<?=$rezervasyon["id"]?>" class="btn"><i class="fa fa-calendar" aria-hidden="true"></i> rezervasyon</a>
